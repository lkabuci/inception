...
wp config create --allow-root --dbname="$WP_DB" --dbuser="$WP_USER" --dbpass="$WP_PASS" --dbhost=172.18.0.2:3306
...
wp config set WP_REDIS_HOST 172.18.0.7 --add --allow-root --path="/var/www/html/"
wp config set WP_REDIS_PORT 6379 --add --allow-root --path="/var/www/html/"
wp config set WP_REDIS_DATABASE 1 --add --allow-root --path="/var/www/html/"
